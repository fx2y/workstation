...
shared_setup_ssh_access() {
    ssh-keygen -t ed25519 -C $WS_USER -f ~/.ssh/$WS_HOSTNAME
    ssh-copy-id -i ~/.ssh/$WS_HOSTNAME -p $WS_PORT $WS_USER@$WS_IP
}
...
shared_setup_gnupg() {
	...
	echo "Disable Internet Connection and Mount Backup Disk"; read -r
	echo "Read KeyID"; read -r KEYID
	echo "Read Backup Directory"; read -r DIR_BAK
	gpg -o - -d "${DIR_BAK}/sub-${KEYID}.gpg" | gpg --import
	echo "Unmount Backup Disk"; read -r
	gpg --list-secret-keys
	gpg --expert --edit-key $KEYID # trust; 5 = I trust ultimately; quit
	gpg -k --with-keygrip
	echo "Read AuthID"; read -r AUTHID
	echo "$AUTHID" >"$HOME"/.gnupg/sshcontrol
	gpg --output ~/.ssh/github --export-ssh-key $KEYID
	echo "Enable Internet Connection and Mount Backup Disk"; read -r
...
EOF
	...
	gpg -K
	echo "Read User Email"; read -r USEREMAIL
	echo "Read User Name"; read -r USERNAME
...
shared_setup_asdf() {
	...
	echo "Read NodeJS Version"; read -r ASDF_NODEJS
	asdf install nodejs $ASDF_NODEJS
	asdf global nodejs $ASDF_NODEJS

	asdf plugin add golang
	asdf list all golang
	echo "Read Golang Version"; read -r ASDF_GOLANG
